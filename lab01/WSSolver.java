...
public class WSSolver {
    ...
    public static void main(String[] args) throws IOException{
        ...
        words = rearrangeSubstrings(words);
    ...
    public static char[][] Resolve(char[][] puzzle, String[] words) {
        ...
        //percorre as palavras e verifica se existem palavras repetidas
        for (int i = 0; i < count; i++) {
            for (int j = 0; j < count; j++) {
                if (words[i].equals(words[j]) && i != j) {
                    System.err.println("Error: Word " + words[i] + " is repeated");
                    System.exit(1);
                }
        ...
        for (int word = 0; word < count ; word++) {
            ...
            for (int y = 0; y < puzzle.length; y++) {
                for (int x = 0; x < puzzle.length; x++) {
                    if (puzzle[x][y] == first_letter && solvedPuzzle[x][y] == '.'){
                        ...
                        if (result != Direction.NONE) {
                            ...
                            //complete the word in the empty puzzle
            ...
            if (repeated > 1) {
                System.err.println(words[word] + " is repeated more times than it should.");
                System.exit(1);
            }

            if (repeated == 0) {
                System.err.println(words[word] + " does not exist in the puzzle.");
                System.exit(1);
            }
    ...
    public static String[] rearrangeSubstrings(String[] words) {
        List<String> substrings = new ArrayList<>();
        List<String> nonSubstrings = new ArrayList<>();
        Set<Integer> addedIndexes = new HashSet<>();

        // Identify and separate substrings and non-substrings
        for (int i = 0; i < count; i++) {
            boolean isSubstringEnd = false;
            for (int j = 0; j < count; j++) {
            ...
            if (isSubstringEnd) {
                ...
                nonSubstrings.add(words[i]);
            }
        }

        // Combine non-substrings and substrings while maintaining their order
        List<String> result = new ArrayList<>(nonSubstrings);
